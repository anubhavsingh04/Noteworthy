...
public class OAuth2LoginSuccessHandler extends SavedRequestAwareAuthenticationSuccessHandler {
    ...
    @Value("${frontend.url}")
    private String frontendUrl; // after success oauth2 authentication redirect user to frontend

    String username;
    String idAttributeKey;

    @Override // post authentication
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws ServletException, IOException {
        OAuth2AuthenticationToken oAuth2AuthenticationToken = (OAuth2AuthenticationToken) authentication;
        // this line should be hit when user is logged in with github
        if ("github".equals(oAuth2AuthenticationToken.getAuthorizedClientRegistrationId()) ||
                "google".equals(oAuth2AuthenticationToken.getAuthorizedClientRegistrationId())) {
            DefaultOAuth2User principal = (DefaultOAuth2User) authentication.getPrincipal(); // this is how user is represented
            ...
            // now all the handling will be done below
            userService.findByEmail(email)
                    // updating security context
                    .ifPresentOrElse(user -> { // user present into the database
        ...
        Set<SimpleGrantedAuthority> authorities = new HashSet<>(oauth2User.getAuthorities().stream()
                ...
                .collect(Collectors.toList())); // github authority part
        ...
        UserDetailsImpl userDetails = new UserDetailsImpl(
                ...
                null,
                false,
                authorities // this will have all the authorities from the github as well as database
                // in the frontend decoded token can be seen {sub: 'anubhavsingh04', roles: 'SCOPE_read:user,user:email,OAUTH2_USER,ROLE_USER',
                // last part is from the database
